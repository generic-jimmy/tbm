"""File info + streaming download from Telegram CDN."""
import httpx
from fastapi import APIRouter, Depends, HTTPException
from fastapi.responses import StreamingResponse

from app.auth import require_auth, require_auth_or_query
...
async def file_info(file_id: str, bot_hash: str,
    ...
    """Fetch Telegram file metadata (Bearer auth only)."""
    token = await _get_token(bot_hash)
    async with httpx.AsyncClient(timeout=10.0) as client:
        res  = await client.get(f"{TG}{token}/getFile", params={"file_id": file_id})
        data = res.json()
        if not data.get("ok"):
...
async def download_file(file_id: str, bot_hash: str,
                        _=Depends(require_auth_or_query)):
    """Stream a file from Telegram CDN to the browser.

    Accepts JWT as Bearer header -or- ?token= query param, because browsers
    hitting this endpoint via <a download> or window.open() cannot inject
    custom headers.
    """
    token = await _get_token(bot_hash)
    async with httpx.AsyncClient(timeout=10.0) as client:
        info = await client.get(f"{TG}{token}/getFile", params={"file_id": file_id})
        d = info.json()
        if not d.get("ok"):
