...
from app.auth import require_auth_or_query
from app.database import db

router = APIRouter(prefix="/api/export", tags=["export"])

FIELDS = [
    "ts", "kind", "sender_name", "sender_id", "chat_id", "chat_title",
...
async def _rows(bot_hash: str, kind: Optional[str], chat_id: Optional[int]):
    return await db.get_messages(bot_hash, kind, chat_id, limit=10_000)


# All three endpoints use require_auth_or_query so the browser can hit them
# directly via <a href="...?token=JWT" download> without custom headers.
...
async def export_xlsx(
    ...
    headers = [
        "Timestamp", "Type", "Sender", "User ID", "Chat ID", "Chat",
